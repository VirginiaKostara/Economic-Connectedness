# Plot graph for Main Figure 5
# IMPORTANT: This program must be run from within Main_Figure_5.do
...
for (school_type in c('high_school', 'college')) {
  ...
  # Import dataset (file path defined in 1. Main_Figure_5.do)
  ...
  if(school_type == 'high_school'){
    ...
    file_name <- paste0(paper2_figs, "/Main_Figure_5a.pdf")
    
    all$exposure_parent_ses <- all$exposure_parent_ses_hs
    all$bias_parent_ses <- all$bias_parent_ses_hs
  } 
  
  else {
    schools <- college_subset
    subtitle_name <- "B. Colleges"
    file_name <- paste0(paper2_figs, "/Main_Figure_5b.pdf")
    
    all$exposure_parent_ses <- all$exposure_parent_ses_college
    all$bias_parent_ses <- all$bias_parent_ses_college
  }
  
  
  # Rescale exposure and bias measures 
  all <- all %>% mutate(exposure_parent_ses = 100 * exposure_parent_ses / 2, 
                        bias_parent_ses = bias_parent_ses * 100) 
  ...
  aes_specs <- aes_string(x = "exposure_parent_ses", y = "bias_parent_ses")
  ...
  plot <- ggplot(all, aes_specs) +
   ...
   # Add graph titles
   title <- textGrob("Friending bias and exposure by high school and college",
